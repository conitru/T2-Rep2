...
#Para el correcto funcionamiento del código, guardar las bases de datos descargadas en la carpeta "data" que se encuentra dentro de la carpeta "docs" del Github, con los mismos nombres con que vienen del link.

#Base 1: Resultados de Votación. Origen: Servel. Dato Público.
Base_Votacion_Gob2024 <- read_excel("data/2024_11_GobernadoresRegionales_Datos_Eleccion.xlsx", 
    skip = 4)
...
Entidad_Ciudadano <- read_csv("data/Entidad Ciudadano (Base Ficticia).csv")
...
Entidad_Elección <- read_excel("data/Base Entidad Elección.xlsx")
...
Proyecciones_INE <- read_excel("data/estimaciones-y-proyecciones-2002-2035-comunas (1).xlsx")
...
CASEN_2022 <- read_excel("data/Estimaciones_Indice_Pobreza_Multidimensional_Comunas_2022.xlsx")
```
...
En este apartado se presenta el análisis preliminar de los primeros hallazgos del análisis, junto con su representación gráfica e interpretación.

### **Distribución de los nuevos votantes según sexo**

1.  Mapa
```{r, results = 'hide'}
comunas <- suppressWarnings(suppressMessages(st_read("comunas.shp")))
...
Comunas_Mapa <- st_as_sf(Comunas_Mapa, coords = c("lon", "lat"), crs = 4326)
...
rm(no_coincidentes,no_coincidentes2)
```
...
Entidad_Ciudadano %>%
  ggplot(aes(x = Tipo_Votante, fill = Sexo)) +  # Eje x para Tipo_Votante, apilando por Sexo
  geom_bar(stat = "count", position = "fill", color = "black") +  # Apilar las barras y normalizar a 100%
  coord_flip() +  # Si prefieres las barras horizontales
  labs(
  ...
  theme_minimal() +  # Tema limpio y sencillo
  theme(
    legend.position = "right",  # Colocar la leyenda a la derecha
    plot.title = element_text(hjust = 0.5)  # Centrar el título
  ) +
  geom_text(
    aes(label = scales::percent(..count../sum(..count..))),  # Calcular el porcentaje
    stat = "count", 
    position = position_fill(vjust = 0.5),  # Colocar las etiquetas en el centro de las porciones
